...
class Login extends Component {
    ...
    changeImg(){
        //获取验证码;
        axios.get({
            url:'https://m.moretickets.com/userapi/pub/v1_1/send_sms_for_regist?',
            data:{
                cellphone: this.phone.value,
                token: '5b461b14e4b07c8e18f4765e',
                time: 1531321121494,
                src: 'm_web'
            }
        }).then((res)=>{
    ...
    //login验证...实际上登陆的时候需要后端的配合，一般都是后端把信息存入cookie;
    login(){
        //手机号码不能为空
        if(!this.phone.value) 
        {
        ...
        if(!isPoneAvailable(this.phone.value))
        {
           alert('请输入正确格式的11位号码!')
           return false;
        }
        if(this.imgCode.value!==this.txtCode.value&&!this.imgCode.value&&!this.txtCode.value)
        {
            alert('请输入正确的验证码!')
            return false;
        }
        let username = this.phone.value;
        let password = this.txtCode.value;
        setTimeout(()=>{
            this.props.codeLogin ({
                username,password,
             success:()=>{
                alert('登陆成功!')
                Cookies.save('username',username,{path:'/',maxAge:new Date().setDate(new Date().getDate()+7)});
    ...
    render(){
        return (
            <div className='login'>
                <Appheader  right = {false} title = '欢迎登陆摩天轮票务网'/>
                    <form>
                        <div>
                            <input  ref = {el=>this.phone = el} type="text"   placeholder="请输入手机换号码"/>
                        </div>
                        <div>
                            <input  ref = {el=>this.imgCode = el} type="text"   placeholder="请输入图形码" className='password'/>
                           <span> <img onClick = {this.changeImg} src = {this.state.url}/></span>
                        </div>
                        <div>
                            <input  ref = {el=>this.txtCode = el} type="text"  placeholder="请输入验证码" className='password'/>
                            <i>{this.renderCode()}</i>
                        </div>  
                    </form>
                        <p><button onClick = {this.login}>登陆</button></p>
...
export default ConnectGroup(Login,['commons'])